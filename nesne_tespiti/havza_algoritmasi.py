...
#KONTUR
contours,hierarchy = cv2.findContours(coin_thresh,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
...
#nesneler arasi mesafe bulalım
dis_transform = cv2.distanceTransform(opening,cv2.DIST_L2,5)
plt.figure(), plt.imshow(dis_transform, cmap ="gray"), plt.axis("off")

#resmi kucult
ret, sure_foreground = cv2.threshold(dis_transform ,0.4*np.max(dis_transform ),255,0)
plt.figure(), plt.imshow(sure_foreground, cmap ="gray"), plt.axis("off")

#arka plan icin resmi buyult
...
#KONTUR
#contours, hierarchy = cv2.findContours(marker,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
